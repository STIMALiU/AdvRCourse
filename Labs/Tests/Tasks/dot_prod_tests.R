...
test_that("dot_prod()", {
  expect_that(exists("dot_prod"), expect_true(), 
  ...
  expect_that(all(names(formals(dot_prod)) %in% c("a", "b")), condition=expect_true(),
              info = "The argument names are incorrect.")
  expect_that(is.numeric(dot_prod(a=1:3, b=4:6)), condition=expect_true(),
