### Assignment : while_mult_table() ###

context("while_mult_table()")

test_that("Assignment: while_mult_table()", {
  
  # Expect object
  expect_true(exists("while_mult_table"), info = "No object 'while_mult_table' exists.")
  
  # Expect class
  expect_is(while_mult_table, class = "function",
            info = "Object 'while_mult_table' is not a function.")

  # Expect arguments
  exp_args <- c("from", "to")
  expect_function_arguments(while_mult_table, expected = exp_args,
  ...
  # Run functions
  test_res <- list()
  test_res[[1]] <- while_mult_table(from = from[1], to=to[1])
  test_res[[2]] <- while_mult_table(from = from[2], to=to[2])

  ## Expect results
  # Expect class
  for (i in seq_along(test_res)){
    expect_is(test_res[[i]], "matrix", 
              info = "'while_mult_table()' do not return a correct object.")
  }
  ...
  for (i in seq_along(test_res)){
    expect_true(all(dim(test_res[[i]]) == dim_res[[i]]), 
                info = "'while_mult_table()' do not return an object with correct dimensions.")
  }
  ...
  for (i in seq_along(test_res)){
    expect_true(all(rownames(test_res[[i]])[1:2] == row_res[[i]]), 
                info = "'while_mult_table()' do not return an object with correct (ordered) variable names.")
  ...
  for (i in seq_along(test_res)){
    expect_true(all(as.vector(test_res[[i]])[1:3] == result_list1[[i]]), 
                info = "'while_mult_table()' returns erroneous results.")
  }
})
